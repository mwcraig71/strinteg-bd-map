...
exports.handler = async (event, context) => {
  ...
  try {
    const res = await fetch(`https://api.notion.com/v1/blocks/${fmtId}/children`, {
      method: 'PATCH',
      headers: {
    ...
    if (!res.ok) {
      ...
      console.error(`Notion error for ${stateKey}:`, err);
      return { statusCode: 500, headers, body: JSON.stringify({ error: 'Notion API error. Check function logs.' }) };
    }
